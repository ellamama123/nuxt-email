      ...
      this.dataSend.find((element) => {
        if(element.id === item.id){
          axios.get('http://127.0.0.1:8000/api/previewMail?id=' +  item.category_mail).then((response) => {
            ...
            this.content = this.content.replace("[Name]", element.name).replace("[dateTime]", this.convertDate(element.dateTime)).replace("[Position]", this.getPosition(element.position));
          });
